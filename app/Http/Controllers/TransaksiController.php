...
class TransaksiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $transaksi->no_reff = $data['no_reff'];
        $transaksi->jenis_saldo = $data['jenis_saldo'];
        $transaksi->saldo = $data['saldo'];
        $transaksi->save();
        if (count($data['keterangan'] > 0)) {
            foreach ($data['keterangan'] as $item => $v) {
                $data2 = array(
                    'tanggal_transaksi' => $data['tanggal_transaksi'][$item],
                    'keterangan'        => $data['keterangan'][$item],
                    'no_reff'           => $data['no_reff'][$item],
                    'jenis_saldo'       => $data['jenis_saldo'][$item],
                    'saldo'             => $data['saldo'][$item]
                );
    ...
    {
        $debet = Transaksi::where('no_reff', 'like', '4%')->orderBy('id', 'asc')->get();
        $kredit = Transaksi::where('no_reff', 'like', '5%')->orderBy('id', 'asc')->get();
        $total_debet = Transaksi::where('no_reff', 'like', '4%')->orderBy('id', 'asc')->sum('saldo');

        $total_kredit = Transaksi::where('no_reff', 'like', '5%')->orderBy('id', 'asc')->sum('saldo');

        $hasil = $total_debet - $total_kredit;

        return view('pages.laba.index', compact('debet', 'kredit', 'total_debet', 'total_kredit', 'hasil'));
    }
}
